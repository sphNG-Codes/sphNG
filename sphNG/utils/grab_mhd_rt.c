...
mean_beta,fluxtot,crosshel,min_b,mean_b,max_b,mean_t,max_t,cen_t,
mean_tr,max_tr,cen_tr;
//char line[120];
char *line = NULL;
size_t len = 0;
...
//    while (getline(line,sizeof(line))>0) {
    while (getline(&line, &len, stdin) > 0) {
	  ...
	  if (total>maxtot) maxtot = total;}
       ...
       else if (sscanf(line, " temperature  mean :%le max:%le cen:%le", &mean_t, &max_t, &cen_t)==3 && objflag<2) {
          printf("%e %e %e ", mean_t, max_t, cen_t);
       }
       else if (sscanf(line, " temper. rad. mean :%le max:%le cen:%le", &mean_tr, &max_tr, &cen_tr)==3 && objflag<2) {      
          printf("%e %e %e ", mean_tr, max_tr, cen_tr);
       }
