...
c
      ...
      WRITE (iprint, 99002)
99002 FORMAT (/,' SPH Kernel : O23 with hfact =',F5.2,/)
c
...
ccc         sum = 3.0/4.0*(10./3. - 7.*v2 + 4.*v3)
         wij(i) = sum
ccc         sum = -3.*v + 2.25*v2
         sum = 3.0/4.0*(-14.*v + 12.*v2)
         grwij(i) = sum
         sum = 1.3333333333*v3 - 1.2*v5 + 0.5*v6
ccc         sum = 10./3.*v3 - 4.2*v5 + 2.*v6
         fmass(i) = sum
         sum = 0.66666666666*v2 - 0.3*v4 + 0.1*v5 - 1.4
ccc         sum = 5./3.*v2 - 21./20.*v4 + 0.4*v5 - 2.1
         fpoten(i) = sum
         sum = -1.4 + 2.*v2 - 1.5*v4 + 0.6*v5
ccc         sum = -2.1 + 5.*v2 - 5.25*v4 + 2.4*v5
         dphidh(i) = sum
      END DO
c
...
ccc         sum = 3.0/4.0*(dif2*dif2*(5. - 4.*v)/3.)
         wij(i) = sum
ccc         sum = -0.75*v2 + 3*v - 3.
     ...
     &         0.0666666666666
ccc         sum = -2./3.*v6 + 4.2*v5 - 9.*v4 + 20./3.*v3 -
ccc     &         0.0666666666666
         fmass(i) = sum
         sum = -0.033333333333*v5 + 0.3*v4 - v3 + 1.3333333333*v2 - 1.6
ccc         sum = -2./15.*v5 + 21./20.*v4 - 3.*v3 + 10./3.*v2 - 2.4
         fpoten(i) = sum
         sum = -1.6 + 4.*v2 - 4.*v3 + 1.5*v4 - 0.2*v5
ccc         sum = -2.4 + 10.*v2 - 12.*v3 + 5.25*v4 - 12./15.*v5
         dphidh(i) = sum
      END DO
c
