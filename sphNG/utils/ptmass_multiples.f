...
c
     ...
     &     xform(idim),yform(idim),zform(idim)
     ...
     &     nbinaryq_12_notrealbd(nbinbin)
      ...
      DIMENSION listmerge(idim),list(idim),indx(idim),listmergen(idim)
...
c         write (*,*) fftime, time, nptmass
      ...
      IF (fftime.GT.stoptime) GOTO 200
...
c            icomp = 147
c            icomp = 181
            IF (i.EQ.59) WRITE (90,455) i,iii_found,
c            IF (i.EQ.140) WRITE (90,455) i,iii_found,
c            IF (i.EQ.175) WRITE (90,455) i,iii_found,
     &           listunique(iii_found),fftime,pmassi,
     &           SQRT((xi-x(icomp))**2+(yi-y(icomp))**2+
     &           (zi-z(icomp))**2)
 455        FORMAT(3(I8,1X),3(1PE12.5,1X))
...
c     &           ptmass4,' spins ',spinx(i), spiny(i), spinz(i)
         ...
         IF (i.GT.nptmassold) THEN
c            WRITE (88,*) fftime,nptmassold
            WRITE (88,99555) time,fftime,nptmass,iuniquenew
99555       FORMAT(2(1PE12.5,1X),I5,1X,I10)
         ...
         spinlength = SQRT(spinx(i)**2 + spiny(i)**2 + spinz(i)**2)
         spinx(i) = spinx(i)/spinlength
         spiny(i) = spiny(i)/spinlength
         spinz(i) = spinz(i)/spinlength
...
c         write (*,*) ipart(i), x(i), y(i)
     ...
     &           oldtimes(i,nlast)
c
c--Accretion rate of 0.0427 is 10^-7 M_sol/yr
c
         IF(endacctime(i).EQ.0.AND.accrete(i).LT.0.0427*1.)THEN
                endacctime(i) = acctime(i)
                WRITE (*,*) 'End acc time ',i, endacctime(i)
            ELSEIF (accrete(i).GE.0.0427*1.) THEN
               endacctime(i) = 0.0
            ENDIF
            IF (time-oldtimes(i,nlast).NE.0.0) THEN
...
99100 FORMAT('# ',2(1PE15.8,1X))
     ...
     &        xform(n),yform(n),zform(n)
99012    FORMAT (26(1PE15.8,1X))
...
ccc         WRITE (99,*) x(n),y(n),z(n)
     ...
     &        pmassmin(nkeep2))
         ...
         WRITE (*,*) 'Unique identifiers: ',listunique(nkeep1),
     &        listunique(nkeep2)
...
88009             FORMAT(11(1PE12.5,1X))
 ...
 18                   FORMAT('4b ',7(1PE12.5,1X))
     ...
     &              spinz(nkeep2)**2)
               spinx1 = spinx(nkeep1)/spinlength1
               spiny1 = spiny(nkeep1)/spinlength1
               spinz1 = spinz(nkeep1)/spinlength1
               spinx2 = spinx(nkeep2)/spinlength2
               spiny2 = spiny(nkeep2)/spinlength2
               spinz2 = spinz(nkeep2)/spinlength2
     ...
     &                 spinz(nkeep2)*planez)/spinlength2)*180.0/pi
                  dotcross = (spiny1*planez-spinz1*planey)*
     &                 (spiny2*planez-spinz2*planey) +
     &                 (spinx1*planez-spinz1*planex)*
     &                 (spinx2*planez-spinz2*planex) +
     &                 (spiny1*planex-spinx1*planey)*
     &                 (spiny2*planex-spinx2*planey)

                  IF (dotcross.LT.0.0) THEN
                     angleorbit2 = - angleorbit2
                  ENDIF

                  angleorbit3 = ACOS(dotcross)*180.0/pi
     ...
     &                 spinlength1/spinlength2)*180.0/pi
                  ...
                  WRITE (*,*) 'Angle Spin-Orbit 3 ',angleorbit3,dotcross
     ...
     &                 period(nkeep2))
               ...
               ENDIF
            ENDIF
            WRITE (50,99444) nbincomp(nkeep1),xmassmax,xmassmin,
     &           interest(nkeep1),interest(nkeep2),
     &           pmassn(nkeep1),pmassn(nkeep2),
     &           qratio_out,semimajorkeep*runit,eccentricitykeep,
     &           angleorbit,angleorbit1,angleorbit2,anglerelative,
     &           periodkeep,periodratiokeep
99444       FORMAT(I2,2(1X,1PE12.5),2(1X,I6),11(1X,1PE12.5))

            IF (nbincomp(nkeep1).EQ.2) THEN
              WRITE (91,98765) name(nkeep1),name(nkeep2),pmassn(nkeep1),
     &              pmassn(nkeep2),qratio,semimajorkeep*runit,
     &              eccentricitykeep,anglerelative,angleorbit1,
     &              angleorbit2
            ELSE
               WRITE (91,98765) name(nkeep1),name(nkeep2),
     &              pmassn(nkeep1),
     &              pmassn(nkeep2),qratio,semimajorkeep*runit,
     &              eccentricitykeep,angleorbit,0.0,0.0
            ENDIF
98765       FORMAT(A15,A15,' & ',F4.2,' & ',F4.2,' & ',F4.2,' & ',
     &           F,' & ',F4.2,' & ',F5.0,' & ',F5.0,' & ',F5.0,' \\')
         ...
         IF (nbincomp(nkeep1).EQ.2) THEN
                  ...
                  WRITE (84,*) '#',name(nkeep1),' ',name(nkeep2)
                  nout1 = listmergen(nkeep1)
                  nout2 = listmergen(nkeep2)
                  IF (nout1.GT.nout2) THEN
                     nout3 = nout1
                     nout1 = nout2
                     nout2 = nout3
                  ENDIF
                  WRITE (84,*) runit*radiuscreate(nout2,nout1),
     &                 runit*radiusend(nout1,nout2),
     &                 runit*radiusend(nout2,nout1)
...
c
         ...
         DO n = 1, nnodes
            IF (n.NE.nkeep2) THEN
               ...
               listunique(ipos) = listunique(n)

               listmergen(ipos) = listmergen(n)
...
98444    FORMAT(I2,3(1X,1PE12.5))
      ...
      CALL indexx(nnodes, list, pmassmax, indx)
      ...
      DO i = 1, 1000
         ...
         DO n = 1, nnodes
            indx_val = indx(n)
c            IF (pmassmax(indx_val).GE.pmass_low .AND. 
c     &           pmassmax(indx_val).LT.pmass_low*xbinsize) THEN
               smooth_fraction = xkernel2( (LOG10(pmassmax(indx_val))-
     &              LOG10(pmass_low)-LOG10(xbinsize)/2.0)/
     &              (0.25/SQRT(2.)*LOG10(xbinsize)) )

               IF (nbincomp(indx_val).EQ.1) THEN
                  xnsingle = xnsingle + 1.0*smooth_fraction
                  nsingle = nsingle + 1
               ELSE
                  xnmultiple = xnmultiple + 1.0*smooth_fraction
                  nmultiple = nmultiple + 1
               ENDIF
c            ENDIF
         END DO
         ntotalsingle = ntotalsingle + nsingle
         ntotalmultiple = ntotalmultiple + nmultiple
         xntotalsingle = xntotalsingle + xnsingle
         xntotalmultiple = xntotalmultiple + xnmultiple
         IF (xnsingle+xnmultiple.GT.0) THEN
            frac = REAL(xnmultiple)/REAL(xnsingle+xnmultiple)
c
c--Use NAG library G07AAF(N, K, CLEVEL, PL, PU, IFAIL)
c     Where CLEVEL is double precision confidence interval (e.g. 0.95 or 0.68)
c
            IF (INT(xnsingle+xnmultiple).GT.0) THEN
               CALL G07AAF(INT(xnsingle+xnmultiple),
     &              INT(frac*(xnsingle+xnmultiple)),
     &              0.68d0, p_upper, p_lower, ifail)
               CALL G07AAF(INT(xnsingle+xnmultiple),
     &              INT(frac*(xnsingle+xnmultiple)),
     &           0.95d0, p_upper95, p_lower95, ifail)
c               p_upper = frac
c               p_lower = 0.
c               p_upper95 = frac
c               p_lower95 = 0.
            ELSE
               ...
               p_lower95 = 0.
            ENDIF
c            p_upp_av = (p_upper + p_upp_last)/2.
c            p_low_av = (p_lower + p_low_last)/2.
c            p_upp95_av = (p_upper95 + p_upp95_last)/2.
c            p_low95_av = (p_lower95 + p_low95_last)/2.
            p_upp_av = p_upper
            p_low_av = p_lower
            p_upp95_av = p_upper95
            p_low95_av = p_lower95
c            WRITE (52,97444) pmass_low*SQRT(xbinsize),pmass_high,
c     &           xnsingle,xnmultiple,
c     &           frac,p_upper,p_lower,p_upper95,p_lower95
            WRITE (52,97444) pmass_low*SQRT(xbinsize),pmass_high,
     &           xnsingle,xnmultiple,
     &           frac,p_upp_av,p_low_av,p_upp95_av,p_low95_av,
     &           nsingle,nmultiple
97444       FORMAT(9(1PE12.5,1X),2(I5,1X))
            p_upp_last = p_upper
            p_low_last = p_lower
            p_upp95_last = p_upper95
            p_low95_last = p_lower95
         ELSE
            frac = 0.
         ENDIF
c         pmass_low = pmass_low*SQRT(SQRT(1.258925411794167))
c         pmass_low = pmass_low*SQRT(1.258925411794167)
         pmass_low = pmass_low*(1.258925411794167)
         IF (pmass_low.GT.100.) GOTO 676
      END DO

 676  WRITE (*,*) 'Totals ',ntotalsingle,ntotalmultiple,xntotalsingle,
     &     xntotalmultiple
...
c
c--This was originally provided by the NAG library, but MRB has
c     written a version using Numerical Recipes routines.
c
      INTEGER N, K, IFAIL
      REAL*8 xn, xk, alpha
      REAL*8 CLEVEL, PL, PU
      ...
      IFAIL = 0
      IF (N.GT.1000000 .OR. K.GT.N) THEN
         IFAIL = -1
         RETURN
      ENDIF
      ...
      IF (k.EQ.0) THEN
         PL = 0.
      ELSE
         PL = beta(xk,xn-xk+1,alpha/2.0)
      ENDIF
      IF (k.EQ.n) THEN
         PU = 1.0
      ELSE
         PU = beta(xk+1,xn-xk,1.0-alpha/2.0)
      ENDIF
      ...
      REAL*8 beta,p,a,b
      REAL*8 betai
      REAL*8 guess,guess_old,delta,func,dfunc
c     
c--N-R interation
c
      guess = a/(a+b)
...
c      print *,guess,p
 ...
 510  func = betai(a,b,guess) - p
c      print *,'func ',func
      IF (ABS(func).LT.1.0E-5) GOTO 520
      delta = 0.001
      IF (delta.GT.guess) delta = guess*0.1
      dfunc = betai(a,b,guess-delta) - p
c      print *,'dfunc ',dfunc
      dfunc = (func-dfunc)/delta

c      print *,guess
      guess_old = guess
      guess = guess - func/dfunc
      IF (guess.LT.0.) guess = guess_old/2.0
      IF (guess.GT.1.0) guess = (1.0+guess_old)/2.0
c      print *,'new guess ',guess
      GOTO 510
...
C  (C) Copr. 1986-92 Numerical Recipes Software Vs1&v%1jw#<?4210(9p#.
 ...
 11   continue
      print *,'a or b too big, or MAXIT too small in betacf ',a,b,x
      stop
 1    betacf=h
      return
      END
